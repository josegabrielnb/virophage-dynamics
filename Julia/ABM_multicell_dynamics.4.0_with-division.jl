...
K = 2000 #cell population carrying capacity
...
function divide_cell(parent_cell,cell_array,time)
    α = rand(Uniform(-2*π,2*π))
    β = rand(Uniform(-2*π,2*π))
    z = parent_cell.z + 2*cell_radius*sin(β)
    x = parent_cell.x + 2*cell_radius*cos(α)*cos(β)
    y = parent_cell.y + 2*cell_radius*sin(α)*cos(β)

    r = 0.0005
    d = r*(1-length(cell_array)/K) #slow down by growth rate
    yes_divide = rand(Binomial(1,d))

    if yes_divide == 1
        new_cell = length(cell_array)+1
        push!(cell_array,cell(time,new_cell,[parent_cell.s[1],1],x,y,z))
    end
end
...
for t = 1:t_max
    ...
    #divide cells
    population = length(cells)
    for i in 1:population
        if cells[i].s[1] == 1 || cells[i].s[1] == 4
            divide_cell(cells[i],cells,t)
        end
    end
