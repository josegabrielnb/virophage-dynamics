...
setwd("~/Desktop/JoseGabriel/PLoS_CompBio/r_figures/time_course_with-division/")
...
p3 <- ggplot()+
  ...
  theme(plot.title = element_text(size = 14, face = "bold",hjust = 0.5), 
        axis.title=element_text(size=12), axis.text = element_text(size=10))+
  theme(legend.position = c(.3,.8),legend.text=element_text(size=10), legend.title=element_text(size=10,face="bold"))+
  scale_color_manual(values = colours)+
  labs(colour="Simulation")+
  ylim(c(0,1200000))
...
cells16_neutral <- read.csv("cvv_dynamics_cells16_neutral.csv")
cells16_inhibition <- read.csv("cvv_dynamics_cells16_inhibition.csv")
cells16_pcd <- read.csv("cvv_dynamics_cells16_pcd.csv")

p41 <- ggplot()+
  geom_line(aes(x=time,y=virophages,colour="Neutral virophages"),data=cells16_neutral)+
  geom_line(aes(x=time,y=virophages,colour="Inhibitory virophages"),data=cells16_inhibition)+
  geom_line(aes(x=time,y=virophages,colour="PCD"),data=cells16_pcd)+
  ylab("Virophage population")+
  xlab("Time step")+
  ggtitle("8-celled organisms")+
  theme_bw()+
...
p7 <- ggplot()+
  ...
  theme(plot.title = element_text(size = 14, face = "bold",hjust = 0.5), 
        axis.title=element_text(size=12), axis.text = element_text(size=10))+
  theme(legend.position = c(.3,.7),legend.text=element_text(size=10), legend.title=element_text(size=10,face="bold"))+
  scale_color_manual(values = colours)+
  labs(colour="Simulation")+
  ylim(c(0,120000))
